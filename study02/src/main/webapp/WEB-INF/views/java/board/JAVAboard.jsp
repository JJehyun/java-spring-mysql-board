...
<!DOCTYPE html>
      <html>
        ...
        <c:url value="/javaboard/Memo" var="Memo" />
         ...
         <c:url value="/javaboard/detail" var="detail" />
...
<li id="liborderbox_boxbox"><img src="<spring:url value='/JAVA/${vvv.file}'/>" width="250px" height="190px" style="border-radius:5px;" /><p><a href="http://localhost:8080/control/javaboard/detail?id=${vvv.id}" style="color:white">${vvv.title}</a></p><p>${vvv.userID}</p><div style="display:flex"><span>조회수: ${vvv.counter} </span><span style="margin-left:10px;">좋아요 ${vvv.good}</span></div><div style="display:flex"><div id="smallboxcolor"></div><div id="smallboxcolor"></div><div id="smallboxcolor"></div></div><span style="margin-top:3px;"> ${vvv.time} </span></li>
 </c:forEach>
</ul>
...
<c:choose> 
	<c:when test="${pagingint eq 1}">
			<a href=""><i> 첫 페이지 </i></a>
			  ...
			  <a href="http://localhost:8080/control/javaboard?paging=${cnt}">${cnt}</a>
			  </c:forEach>
	   		<a href="http://localhost:8080/control/javaboard?paging=${cnt+1}"><i>NEXT</i></a>
	</c:when> 



	<c:when test="${pagingint eq totalwrite}">
			<a href="http://localhost:8080/control/javaboard?paging=${cnt-1}"><i> PREV </i></a>
			  <c:forEach var="cnt" begin="${pagingint-2}" end="${pagingint}" step="1">
			  <a href="http://localhost:8080/control/javaboard?paging=${cnt}">${cnt}</a>
			  </c:forEach>
	   		<a href=""><i>마지막페이지</i></a>
	</c:when> 
